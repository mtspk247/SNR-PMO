...
import { DriveFolder, DriveGrant, listDriveGrants, upsertUserGrant, removeDriveGrant } from '@/lib/db';

const LEVELS = [{ value: 'viewer', label: 'Viewer' }, { value: 'commenter', label: 'Commenter' }, { value: 'editor', label: 'Editor' }];

// Per-item access overview: who can reach this file/folder, split into INHERITED
// (from the drive or a parent folder, read-only here) and DIRECT (granted on this
// item — editable). Inheritance flows down, so a grant here also covers everything inside.
export default function DriveAccessModal({ target, orgId, folders, people, meId, canManage, onClose }: {
  ...
  useEffect(() => { load(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [target.drive_id, target.id]);
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center mb-1">
              ...
              {canManage && sel.size > 0 && <button className="text-2xs text-rose-600 hover:underline" disabled={busy} onClick={bulkRemove}>Remove {sel.size} selected</button>}
            </div>
            <div className="rounded-lg border border-line divide-y divide-line">
              ...
              {explicit.map((g) => (
                <div key={g.id} className="flex items-center gap-2 px-3 py-2 text-sm">
                  {canManage && <input type="checkbox" className="shrink-0" checked={sel.has(g.id)} onChange={() => toggleSel(g.id)} />}
