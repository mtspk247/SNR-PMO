...
import { getTenantUsage } from '@/lib/db';
 ...
 */
interface Step { key: string; label: string; hint: string; href: string; icon: string; done: boolean }
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        {steps.map((s) => (
          <Link key={s.key} href={s.href}
            className={`flex items-center gap-3 rounded-lg border p-3 transition ${s.done ? 'border-line bg-surface2/40' : 'border-line hover:border-accent/40 hover:bg-surface2'}`}>
