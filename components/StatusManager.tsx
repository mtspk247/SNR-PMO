  ...
  return (
    <Modal open={open} onClose={onClose} title="Manage statuses" subtitle="Customize the workflow statuses for this workspace." icon="ti-flag-3" size="md"
      ...
      <div className="space-y-2">
        {statuses.map((st) => (
          <div key={st.id} className="flex items-center gap-2">
            ...
            <div className="w-24"><Select value={st.category} onChange={(v) => patch(st.id, { category: v })} className="shrink-0" options={[{ value: 'todo', label: 'To-do' }, { value: 'active', label: 'Active' }, { value: 'done', label: 'Done' }]} /></div>
      ...
      <div className="flex items-center gap-2 mt-4 pt-4 border-t border-line">
        ...
        <div className="w-24"><Select value={cat} onChange={(v) => setCat(v)} className="shrink-0" options={[{ value: 'todo', label: 'To-do' }, { value: 'active', label: 'Active' }, { value: 'done', label: 'Done' }]} /></div>
