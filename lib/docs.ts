...
export const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    icon: 'ti-credit-card',
    blocks: [
      { kind: 'p', text: 'Upgrade prompts (configurable): the app can show upgrade nudges — banners, feature lock screens and more — to encourage moving to a higher plan. Platform owners manage the defaults under Platform ▸ Upgrade prompts (add, edit, pause or remove any prompt, choose the trigger — always-on promo, storage/usage threshold, feature locked, seat limit — placement, audience and button link). Resellers can tailor these for their own sub-tenants under Reseller ▸ Upgrade Prompts, overriding or pausing the platform defaults and adding their own. Prompts are resolved per tenant (platform defaults + reseller overrides) and never shown for something the tenant already has.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'steps', items: [
        { title: 'Record your screen', body: 'Open Work ▸ Screen Recordings and click “New recording”. Optionally tick “Include microphone” to narrate, then Start recording and choose the tab, window or whole screen to share. A live timer shows the elapsed time; you can Pause and Resume at any point; recordings stop automatically at 10 minutes (max 200 MB). Click Stop (or the browser’s “Stop sharing”) when done.' }, { title: 'Choose quality, mic & webcam', body: 'Before recording, pick a quality preset (High, Balanced, or Light for a smaller file), and optionally turn on your microphone (to narrate) and a webcam bubble (picture-in-picture in the corner). A 3-2-1 countdown gives you time to get ready.' },
        ...
        { title: 'Watch, rename & download', body: 'Click any row to play the recording in the browser, download the .webm file, or delete it. Rename inline from the list. Use search, columns and CSV export like every other list.' }, { title: 'Share a link', body: 'Open a recording and click Share link to get a public viewer link (like Loom) you can send to anyone — no login needed. The link streams the video read-only via a short-lived secure URL; revoke it anytime and it stops working. Great for demos, bug reports and client walkthroughs.' }, { title: 'Edit before saving', body: 'After recording, click Edit to trim the start/end, speed it up into a timelapse (2×–8×), adjust brightness, crop to 16:9/4:3/square, and add a watermark — then save as video (WebM) or export an animated GIF. Editing happens right in your browser.' },
      ] },
      { kind: 'bullets', items: [
        ...
        { text: 'Thumbnails — a poster image is captured automatically and shown in the list and as the player’s preview frame.' },
        { text: 'Attach to a task — open a recording and attach it to any task (e.g. a bug repro or walkthrough); detach anytime. Great for linking a screen capture to the work it explains.' },
        { text: 'Malware-scanned — every recording (and its thumbnail) is virus-scanned before it can be played or downloaded, the same server-enforced gate used for Drive files.' },
      ] },
    ],
  },
  {
    ...
    blocks: [
      ...
      { kind: 'p', text: 'Media Library: under Marketing > Social Media > Media Library, keep reusable images, video and GIFs. Add media three ways: Upload straight from your device (files are malware-scanned before they can be used), pick From Drive (any image/video already in your Drives — already scanned, so it is ready instantly), or Add by URL (a direct https link from a CDN or host). Uploaded and Drive media are stored privately in your workspace and shown via short-lived secure links. When composing a post, pick any media to attach it. Assets are org-scoped and RBAC-gated like every other list.' },
