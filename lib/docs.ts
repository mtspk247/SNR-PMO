...
export const SECTIONS: Section[] = [
  {
    ...
    blocks: [
      ...
      { kind: 'steps', items: [
        ...
        { title: '8. Start with sample data (optional)', body: 'When you first create a workspace, the welcome screen offers to add sample data so you can explore — leave it ticked to land in a fully populated workspace (example projects, deals, invoices and live dashboards, plus a starter AI-agent team on plans that include Agents) instead of an empty one; untick it for a clean start. Owners can load realistic sample data from Settings ▸ Profile ▸ “Generate demo data” to see every module populated. You can also add \u201cSample smart columns\u201d there to see relationship, rollup and formula columns working on real Clients data. You can wipe it later from Settings ▸ Danger zone.' },
      ] },
