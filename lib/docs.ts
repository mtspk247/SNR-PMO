...
export const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'callout', icon: 'ti-route', text: 'Run a workflow: on the Agents page, managers can pick a ready-made workflow (Onboard a new client, Kick off a project), fill a couple of fields, and the agent proposes the whole thing as an ordered, approve-first plan \u2014 for example a client onboarding becomes a CRM contact, an onboarding project with starter tasks, and the sales opportunity, all queued together. Every step is dry-run + preflighted in Agent approvals, and nothing runs until you approve it. It is the fastest way to turn a repeatable back-office process into one reviewed click, and every action stays reversible. You can also just type it in plain English in the “Ask your agent” box — for example “onboard Acme Corp” or “kick off project Apollo” — and your agent turns the request straight into the same preflighted, approve-first plan (add the name in the form if it needs one).' },
