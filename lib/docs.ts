...
export const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'steps', items: [
        ...
        { title: 'Set page-by-page access (Create / View / Edit / Delete)', body: 'On a role (Users then Roles) or a person (their Access tab) open Page access — a collapsible tree of every module (Work, People, Accounting and so on) and the pages under it, each with Create / View / Edit / Delete boxes. Uncheck View to stop that role or user seeing a page; it disappears from their sidebar and search. A per-user setting overrides the role template. View is enforced now; Create / Edit / Delete enforcement rolls out per module.' },
      ] },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'callout', icon: 'ti-shield-lock', text: 'Security: secrets are stored encrypted server-side and are never displayed or sent to the browser \u2014 the page shows status only, never the key value. These provider keys carry no fixed expiry; rotate them periodically by re-entering the key on each integration\u2019s manage page. You can also set a per-key rotation-reminder date on the page \u2014 it shows \u201cdue in Nd\u201d as the date nears and \u201cOverdue\u201d once it passes.' },
    ],
  },
  {
    id: 'connections',
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'callout', icon: 'ti-shield-lock', text: 'Permissions: by default any staff member can open a drive and add files, while renaming, moving or deleting stays limited to the item’s creator (or an owner/admin). For finer control, Restrict a drive and grant people Viewer, Commenter or Editor access — Editors edit and upload, Commenters can only comment, Viewers are read-only. Every rule is enforced on the server by row-level security, not just hidden in the UI. Drives is plan-gated and storage counts against your plan quota.' },
    ],
  },
  {
    id: 'forms',
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'callout', icon: 'ti-radar', text: 'No LLM key? Open an Accounting, Tasks, CRM, People or Support agent and click "Find work in my data" \u2014 the agent scans your real records (uncategorized expenses, overdue tasks, stale deals, capacity risks, unassigned tickets) and proposes concrete actions you can approve, or that auto-run if the agent is in Auto low-risk mode. An LLM key adds free-form natural-language requests on top.' },
      ...
      { kind: 'callout', icon: 'ti-bolt', text: 'Agents take real actions, not just drafts: create and assign tasks, draft onboarding checklists, post journal entries, categorize expenses, triage tickets and support replies, move deal stages, send approved SMS, create CRM contacts and deals, scaffold a whole project with its starter tasks, and onboard a new client end-to-end (contact + project + tasks) \u2014 each one proposed, approved with a click, audited, and one-click reversible.' },
